    ...
    while True:
        ...
        while True:
            ...
            if not point_move_list:
                ...
                fail_check +=1
                pos_data.append(["None", "None", str(i), str(p)])
                np.savetxt("pos_independent_2.txt", pos_data, fmt="%s")
                continue
            ...
            for pole in vybrane_pole:
                ...
                if np.linalg.norm(new_pos - prev_pos) > threshold:
                    ...
                    if p <= 100:    
                        pos_data.append([str(check_pos), str(new_pos), str(i), str(p)])
                        np.savetxt("pos_independent_2.txt", pos_data, fmt="%s")
                    
                    if p > 100:
                        action_test = torch.tensor([x,y,z], dtype=torch.float32, device=device).unsqueeze(0)
                        with torch.no_grad():
                            pred_state_test = world_model(state_t, action_test) 
                        pred_state_test = torch.cat([pred_state_test.squeeze(0).detach()]).cpu().numpy()
                        predicted_real = pred_state_test * (limits_max - limits_min) + limits_min
                        future_point_pose = np.array([predicted_real[0], predicted_real[1], predicted_real[2]])
                        pos_data.append([str(future_point_pose), str(new_pos), str(i), str(p)])
                        np.savetxt("pos_independent_2.txt", pos_data, fmt="%s")
        ...
        np.savetxt("cur_wm_independent_2.txt", results_array, fmt="%s")
